...
class SportsLiveBarrageWidget extends StatefulWidget {
  const SportsLiveBarrageWidget({Key? key}) : super(key: key);

  @override
  State createState() => _SportsLiveBarrageWidgetState();
}

class _SportsLiveBarrageWidgetState extends State<SportsLiveBarrageWidget> {
  final List<_BarrageItem> _allText = [];
...
class _BarrageItem extends StatelessWidget {
  const _BarrageItem({Key? key, this.top = 0, this.text, this.onComplete})
      : super(key: key);
  final double top;
  final String? text;
...
class _AnimalWidget extends StatefulWidget {
  const _AnimalWidget({required this.child, this.onComplete});
  final Widget child;

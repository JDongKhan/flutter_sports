...
class SportsListPlayer extends StatefulWidget {
  ...
  @override
  State createState() => _SportsListPlayerState();
}
...
class _SportsListPlayerState extends State<SportsListPlayer>
  ...
  void _listener() {
    ...
    if (mounted && !(context.owner?.debugBuilding ?? false)) {
      if (_playing != p) {
        setState(() {
          _playing = _sportsListPlayerController
    ...
    return _playing && _visiable
        ...
        : Container(
            child: Stack(
              ...
              children: [
                Image.asset(
                  AssetBundleUtils.getImgPath(widget.cover),
                  fit: BoxFit.fitWidth,
                ),
                IconButton(
                  icon: const Icon(

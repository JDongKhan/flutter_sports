...
import '../../../utils/relative_date_format_utils.dart';
import '../../../widgets/direction_button.dart';
import '../../article/page/sports_article_detail_page.dart';
...
class SportsHomeItem extends StatelessWidget {
  const SportsHomeItem({required this.content});
...
class SportsHomeItem1 extends StatefulWidget {
  const SportsHomeItem1({required this.content});
  final SportsContent content;
  @override
    ...
    return GestureDetector(
      ...
      child: ListTile(
        ...
        trailing: Image.asset(
          AssetBundleUtils.getImgPath(fileName, format: fileSuffix),
          width: 100,
          fit: BoxFit.fill,
        ),
      ),
...
class SportsHomeItem2 extends StatefulWidget {
  const SportsHomeItem2({required this.content});
  final SportsContent content;
  @override
...
class _SportsHomeItem2State extends State<SportsHomeItem2> {
  ...
  Widget build(BuildContext context) {
      ...
      userInfoWidget = Row(
        children: [
          ClipRRect(
            ...
            child: Image.asset(
              AssetBundleUtils.getImgPath(widget.content.authorImageUrl,
                  format: 'png'),
              width: 50,
              height: 50,
              fit: BoxFit.fill,
    ...
    if (widget.content.source?.isNotEmpty ?? false) {
      tips = widget.content.source ?? '';
      tips += ' ';
    }

    // ignore: null_aware_in_condition
    if (widget.content.commentNum?.isNotEmpty ?? false) {
      tips += widget.content.commentNum ?? '';
      tips += '评论 ';
    }

    // ignore: null_aware_in_condition
    if (widget.content.time?.isNotEmpty ?? false) {
      tips +=
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Image.asset(
            AssetBundleUtils.getImgPath(widget.content.img, format: 'png'),
            fit: BoxFit.fitWidth,
...
class SportsHomeItem3 extends StatefulWidget {
  const SportsHomeItem3({required this.content});
  final SportsContent content;
  @override
...
class _SportsHomeItem3State extends State<SportsHomeItem3> {
  ...
  Widget build(BuildContext context) {
    Widget userInfoWidget = Container();
    if (widget.content.authorName?.isNotEmpty ?? false) {
      userInfoWidget = Row(
        children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(25),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  RelativeDateFormatUtils.formatFromString(
                      widget.content.time ?? ''),
                  style: const TextStyle(
                    fontSize: 12,
                    color: Colors.grey,
...
class SportsHomeItem4 extends StatefulWidget {
  const SportsHomeItem4({required this.content});
  final SportsContent content;
  @override
        ...
        children: [
          Expanded(
            child: ListView.builder(
              itemCount: widget.content.videos?.length ?? 0,
              itemExtent: 200,
                ...
                return Container(
                  ...
                  child: Stack(
                    children: [
                      Positioned.fill(
                        child: Image.asset(
                          AssetBundleUtils.getImgPath(videos.imgUrl,
                              format: 'png'),
                          fit: BoxFit.fitWidth,
                          width: 200,
                        ),
...
class SportsHomeItem5 extends StatelessWidget {
  const SportsHomeItem5({required this.content});
  final SportsContent content;
  @override
  Widget build(BuildContext context) {
    return Container(
